Warning: truncated output (original token count: 36237)
		...
		plugin.getAdminVoteCommand().add(new CommandHandler(plugin, new String[] { "VoteSite", "(sitename)", "Create" },
			...
			public void execute(CommandSender sender, String[] args) {
				...
				if (plugin.getConfigVoteSites().tryGenerateVoteSite(args[1])) {
					...
					sender.sendMessage(MessageAPI.colorize("&cUnable to create VoteSite: unsupported name"));
				}
			...
			public void execute(CommandSender sender, String[] args) {
				...
				plugin.getBukkitScheduler().runTaskAsynchronously(plugin, new Runnable() {
					...
					public void run() {
						...
						case FAIL_SPIGOT: {
							sender.sendMessage(MessageAPI
									.colorize("&cFailed to check for upd…6237 tokens truncated…atabase of offline UUIDs, keeps online UUIDs", true, true) {
