...
public final class HostedControlManager implements AutoCloseable {
	...
	/**
	 * Installs a verifier generated by a backend connected through the trusted
	 * proxy plugin-message channel. The raw credential never reaches this process.
	 */
	public CompletableFuture<Boolean> installNodeVerifier(String nodeId, String verifier) {
		if (nodeId == null || !nodeId.matches("[A-Za-z0-9][A-Za-z0-9._-]{0,63}")
				|| verifier == null || !verifier.matches("[0-9a-f]{64}")) {
			return CompletableFuture.completedFuture(false);
		}
		String key = nodeId + ':' + verifier;
